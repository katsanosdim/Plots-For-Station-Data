...
with st.sidebar:
    ...
    st.header("Fonts")
    axis_fontsize = st.slider("Axis Font Size", 6, 24, 10)
    title_fontsize = st.slider("Title Font Size", 8, 32, 16)

    st.header("Colorbar & Title")
    custom_title = st.text_input(
        "Custom Title", value="Data - Area Sum Precipitation % Anomaly for"
    )
...
if uploaded_file is not None:
    try:
                ...
                for i, year in enumerate(df['Year']):
                    if year % year_step == 0:
                        ax.text(
                            ...
                            fontsize=axis_fontsize, alpha=0.7
                        )
        ...
        plt.title(f"{custom_title} {parameter} ({min_year}-{max_year})", fontsize=title_fontsize)

        ax.tick_params(axis='y', labelsize=axis_fontsize)

        sm = ScalarMappable(norm=norm, cmap=cmap)
        cbar = fig.colorbar(sm, ax=ax, orientation='vertical', fraction=0.03, pad=0.02)
        cbar.set_label("% Anomaly", fontsize=axis_fontsize)
